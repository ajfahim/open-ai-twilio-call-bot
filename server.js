...
// Only use express-ws in development
...
app.all("/incoming-call", (req, res) => {
  const wsPath = process.env.NODE_ENV === 'production' ? '/api/websocket' : '/media-stream';
  const twimlResponse = `<?xml version="1.0" encoding="UTF-8"?>
                          <Response>
                              ...
                              <Connect>
                                  <Stream url="wss://${req.headers.host}${wsPath}" />
                              </Connect>
                          </Response>`;
...
// WebSocket route for media-stream (development only)
if (process.env.NODE_ENV !== 'production') {
